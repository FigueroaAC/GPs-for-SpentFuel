...
def alpha_calculator(Params,X,Y):
    ...
    #print(np.linalg.eigvalsh(KSelf)) # --> Check Eigenvalues of Matrix
...
def GPR_MODEL_w_Std(Params,Lambda_Inv,Xtrain,Ytrain,alpha_,K_inv,Xtest):#,tconstant):
    Constant = Params[0]
    sigma = Params[-1]
...
X = np.load('/Users/AFigueroa/Desktop/GPR-Paper/Training_Sets/X_Candu_Sobol625.npy',
            allow_pickle=True)
Y = np.load('/Users/AFigueroa/Desktop/GPR-Paper/Training_Sets/YTotal_Output_Sobol.npy',
            allow_pickle=True).item()

XtestG = np.load('/Users/AFigueroa/Desktop/GPR-Paper/Training_Sets/X_Candu_Grid625.npy',
            allow_pickle=True)
...
for isotope in tqdm(isotopes):
    try:
        Kernel = np.load('/Users/AFigueroa/Desktop/GPR-Paper/Kernels-Mass/Sobol/{}.npy'.format(isotope),
                         allow_pickle=True).item()
        size=100
